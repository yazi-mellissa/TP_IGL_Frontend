...
import handleSignUp from "@/data/auth/handleSignUp";
...
const SignUp = () => {
	...
	const onSubmit = async (data) => {
		await handleSignUp(data.email, data.password, data.nom).then((res)=>{
			if ( res.status ){
				router.replace('/')
			}
			setError(res);
	...
	return (
		<div className="w-screen h-screen flex justify-center items-center bg-aa-vert">
			<div className="relative w-full h-full md:w-[95%] md:h-[90%] flex flex-col md:flex-row items-center bg-aa-vert justify-start md:justify-center pt-10 md:pt-0">
				...
				<div className=" w-[90%] h-fit md:w-[50%] md:h-full bg-aa-marron rounded-xl z-10 md:z-0 md:rounded-3xl md:px-[7%] md:-ml-[5%] flex flex-col gap-4 md:gap-10 md:pt-10 p-5 md:p-0 md:overflow-y-scroll hide-scroll-bar">
					...
					<div className=" w-full flex flex-col justify-evenly items-start gap-3">
						<h2 className="text-aa-vert font-bold text-2xl md:text-3xl">Inscrivez-vous</h2>
						<p  className="text-[#252620] font-medium text-base md:text-lg">Créer votre compte</p>
					</div>
					...
					<form className=" flex flex-col w-full gap-4 md:gap-10" onSubmit={handleSubmit(onSubmit)}>
						<div className="relative flex flex-col justify-between items-start gap-2">
							<label className="text-black text-lg font-medium">Nom :</label>
							<input
								type="text"
					...
					<div className=" text-[#252620] font-medium flex justify-center items-center text-base text-center md:text-lg">
						...
						vous avez deja un compte ?&nbsp;
							<span>
								<a href="/login" className="text-aa-vert hover:underline">connectez-vous</a>
							</span>
						</span>
